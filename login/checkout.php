...
<head>
  <meta charset="utf-8">
  <title>Checkout - AgroLinked</title>
...
<body class="page bg-image">
      ...
      <form id="checkoutForm">
        <div class="input-group">
          ...
          <select name="hub_id" required>
            <?php 
              $db=(new DB())->connect(); 
              $hubs = $db->query("SELECT * FROM hubs ORDER BY hub_name")->fetchAll(); 
              foreach($hubs as $h) {
                echo "<option value=\"{$h['hub_id']}\">".htmlspecialchars($h['hub_name'].' — '.$h['region'])."</option>"; 
              }
            ?>
          </select>
        </div>
        ...
        <div class="input-group">
          ...
          <input name="delivery_fee" type="number" step="0.01" value="10.00" required>
        </div>
  ...
  <script>
    document.getElementById('checkoutForm').addEventListener('submit', async (e)=>{
      ...
      if (cart.length===0){ 
        document.getElementById('msg').textContent='Cart empty'; 
        return; 
      }
