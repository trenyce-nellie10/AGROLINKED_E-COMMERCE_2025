...
function render(){
  ...
  if(cart.length===0){ el.innerHTML='<p>Cart empty</p>'; document.getElementById('total').textContent=''; return; }
  el.innerHTML = cart.map((c,i)=>`<div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #eee;"><div>${c.title} x ${c.qty}</div><div>₵ ${ (c.price*c.qty).toFixed(2) }</div></div>`).join('');
